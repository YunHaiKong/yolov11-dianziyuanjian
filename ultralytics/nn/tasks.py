    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        if "nn." in m:
            ...
            try:
                ...
                # Handle special case for RepC3k2 if not in globals
                if m == 'RepC3k2':
                    m = RepC3k2
                else:
                    raise KeyError(f"Module '{m}' not found in globals")
        ...
        if m in {
            ...
            if m is C3k2 and scale in "mlx":  # for M/L/X sizes
                args[3] = True
            if m is RepC3k2 and scale in "mlx":  # for M/L/X sizes
                args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
