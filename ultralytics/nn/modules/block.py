...
class RepC3k2(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            # C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
            RepCSP(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )

# RepCSP replace C3k； C3K和RepCSP都是继承C3
...
class LKPSABlock(nn.Module):
    ...
    def __init__(self, c, attn_ratio=0.5, num_heads=4, shortcut=True) -> None:
        ...
        # self.attn = deformable_LKA(c, attn_ratio=attn_ratio, num_heads=num_heads)
        self.attn = deformable_LKA(c)
        self.ffn = nn.Sequential(Conv(c, c * 2, 1), Conv(c * 2, c, 1, act=False))
        self.add = shortcut
...
class LKC2PSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.Sequential(*(LKPSABlock(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n)))
...
class TriC2PSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.Sequential(*(TriPSABlock(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n)))
