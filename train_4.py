...
def train_yolo():
    ...
    # 开始训练
    results = model.train(
        data='ultralytics\cfg\datasets\dianziyuanjian.yaml',  # 数据集配置文件
        epochs=100,            # 训练轮数
        imgsz=640,           # 图片尺寸1
        batch=8,             # 批次大小2
        device='0',        # 使用GPU训练
        ...
        plots=True,          # 生成训练图表
        val=True,           # 暂时关闭验证
        project='runs/Dianziyuanjian_4', # 保存目录
        name='train', # 实验名称
    )
...
def validate_model(model_path='runs/train/Dianziyuanjian/weights/best.pt'):
    """验证模型"""
...
def predict_sample(model_path='runs/train/Dianziyuanjian/weights/best.pt', image_path='images'):
    """使用训练好的模型进行预测"""
    if os.path.exists(model_path):
        model = YOLO(model_path)
            ...
            for r in results:
                ...
                if len(r.boxes) > 0:
                    for box in r.boxes:
                        ...
                        class_names = ['聚酯电容', '热敏电阻', '三极管损坏', '三极管完好']
